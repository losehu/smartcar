...
int16 OFFSET0 = 0;      //最远处，赛道中心值综合偏移量
int16 OFFSET1 = 0;      //第二格
int16 OFFSET2 = 0;      //最近，第三格
int16 TXV = 0;          //梯形的左高度，右高度
...
void Get_Use_Image(void)
{
    ...
    for (i = 0; i < IPS114_H; i ++)          //神眼高 120 / 2  = 60，
        // for (i = 0; i < IMAGEH; i += 3)       //OV7725高 240 / 3  = 80，
    {
        for (j = 0; j <= IPS114_W; j ++)     //神眼宽188 / 2  = 94，
            // for (j = 0; j <= IMAGEW; j += 3)  //OV7725宽320 / 3  = 106，
        {
...
void Get_Bin_Image(unsigned char mode)
{
    ...
    if (mode == 1)
    {
        ...
        Threshold1 = (unsigned short)(tv / 60 / 128);  //求平均值,光线越暗越小，全黑约35，对着屏幕约160，一般情况下大约100
        Threshold1 = Threshold1 + 20;      //此处阈值设置，根据环境的光线来设定
    }
    //    else if (mode == 2)
    //    {
    //        Threshold = 80;                          //手动调节阈值
    //        lq_sobel(CRM, Bin_Image, (unsigned char) Threshold);
    ...
    //    else if (mode == 3)
    //    {
    //        lq_sobelAutoThreshold(CRM, Bin_Image);  //动态调节阈值
    //        return;
    //    }
    /*
    #ifdef USEOLED
 ...
 * @date     2020/5/15
 */
void lq_sobel(unsigned char imageIn[IPS114_H][IPS114_W], unsigned char imageOut[IPS114_H][IPS114_W], unsigned char Threshold)
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
 ...
 * @date     2020/5/15
 */
void lq_sobelAutoThreshold(unsigned char imageIn[IPS114_H][IPS114_W], unsigned char imageOut[IPS114_H][IPS114_W])
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
    short xStart = KERNEL_SIZE / 2;
