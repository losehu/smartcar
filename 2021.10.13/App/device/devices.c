...
void Encoder_Read(void)
{
    ...
    /**********计算左轮参数**********/
    //真实速度
    L_Wheel.now = (float)L_Wheel.encoder * 0.2142;
    ...
    //限幅
    L_Wheel.out = range_protect_float(L_Wheel.now, 0, 500);
    //记录
    L_Wheel.last = L_Wheel.now;
    /**********计算右轮参数**********/
    //真实速度
    R_Wheel.now = (float)R_Wheel.encoder * 0.2142;
    ...
    //限幅输出
    R_Wheel.out = range_protect_float(R_Wheel.now, 0, 500);
    //记录
    ...
    //  {
    //      Wheel.now = L_Wheel.out - ICM_Treated.gyro.z * 0.17;
    //  }
    //  else if(ICM_Treated.gyro.z > 50)//左转取右
    //  {
    //      Wheel.now = R_Wheel.out + ICM_Treated.gyro.z * 0.17;
    //  }
    //  else
    //  {
...
uint8 Encoder_Show(void)
{
    while (KeyPageUp != onepress)
    {
        ...
        ips114_showint16(120, 1, L_Wheel.now);
        ips114_showint16(120, 2, R_Wheel.encoder);
        ips114_showint16(120, 3, R_Wheel.now);
        ips114_showint16(120, 4, Wheel.now);
        IPS114_PENCOLOR = RED;
        delay_ms(10);
    }
    KeyPageUp = nopress;
