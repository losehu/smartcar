...
//{
//    KalmanFilter(ICM_Treated.accdata.x*90 ,ICM_Treated.gyro.x);
//}
...
uint8 ICM20602_Show(void)
{
    while(KeyPageUp != onepress)
    {
        ...
        ips114_showstr(0, 3, "gyro_x");
        ips114_showstr(0, 4, "gyro_y");
        ips114_showstr(0, 5, "gyro_z");

        IPS114_PENCOLOR = BLUE;
        ips114_showfloat(120, 0, ICM_Treated.accdata.x, 4, 1);
        ips114_showfloat(120, 1, ICM_Treated.accdata.y, 4, 1);
        ips114_showfloat(120, 2, ICM_Treated.accdata.z, 4, 1);
        ips114_showfloat(120, 3, ICM_Treated.gyro.x, 4, 1);
        ips114_showfloat(120, 4, ICM_Treated.gyro.y, 4, 1);
        ips114_showfloat(120, 5, ICM_Treated.gyro.z, 4, 1);
...
//===============================四元数============================================
#define Kp 1.2f //10.0f             	// proportional gain governs rate of convergence to accelerometer/magnetometer
...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
    ...
    //	Attitude_Angel_Y = fast_atan2(2*q2*q3 + 2*q0*q1, -2*q1*q1 - 2*q2*q2 + 1)*57.3; // roll

    //	Attitude_Angel_Z = fast_atan2(2*q1*q2 + 2*q0*q3, -2*q2*q2 - 2*q3*q3 + 1)*57.3; // yaw
    //	Attitude_Angle.Z = 0;
}
