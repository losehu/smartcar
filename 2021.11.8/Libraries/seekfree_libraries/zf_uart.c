...
//-------------------------------------------------------------------------------------------------------------------
void uart_init_zf(UARTN_enum uartn, uint32 baud, UARTPIN_enum tx_pin, UARTPIN_enum rx_pin)
{
...
PUTCHAR_PROTOTYPE
{
    while ((UART3->CSR & UART_CSR_TXC) == 0); //The loop is sent until it is finished
    UART3->TDR = (ch & (u16)0x00FF);
    return ch;
}
#else
