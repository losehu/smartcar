...
//需要设置
#define   WAVEFORM_COUNT   10        //有几路波形 会发送前x路波形  

short  waveform[20];         //波形缓存
unsigned char data_to_receive[100]; //接收的缓存
char data_to_send[100];	//发送数据缓存    
 ...
 * @example: none
 */
void Waveform_Write(void)//数据类型int16
{
    waveform[USERDATA_1]   = (int16)(Wheel.LSpeed);
	waveform[USERDATA_2]   = (int16)(Wheel.RSpeed);
    waveform[USERDATA_3]   = (int16)(Wheel.now);
...
//		data_to_send[_cnt++]=0x00;
		...
		for(unsigned char i=0;i<data_to_send[3] + 4;i++)
		{
			Sum_check += data_to_send[i];
			Additional_check += Sum_check;
		}
		data_to_send[_cnt++]=Sum_check;
		data_to_send[_cnt++]=Additional_check;
		//串口发送数据 ///////////////////写在这里
