...
void Timer_IT(void)  //TM6中断
{
    ...
    if (TIMx_Cnt % 100 == 0 && CarInfo.Protect == 'F')
    {
        Encoder_Read();
        // BalanceControlOut =BalancePIDParam[0][0]*(Attitude_Angel_P-set_angle);

        BalanceControlOut = Float_SimpleIncremental_PID(&BalancePIDParam[0][0], &BalancePID, set_angle, Attitude_Angel_P);
        //BalanceControlOut=abs(BalanceControlOut)>10?BalanceControlOut:0;

        Balance_L_Out = -Float_SimpleIncremental_PID(&LeftMotorSpeedPIDParam[0][0], &BalancePID, BalanceControlOut, L_Wheel.now);


        Balance_R_Out = Balance_L_Out;
