...
int kk = 24;
int Radius = 0; //PID偏差
float setted_angle = 33;
int  Direct_Parameter = 0;// 转向系数;
int Direct_Last = 0;
int start_line=10,end_line=30;///////////////10-25
...
void Timer_IT(void)  //TM6中断
{
    ...
    if (TIMx_Cnt % 5 == 0 )
    {
        angle_out = -30 * PID_Realize2(&Angle_PID, Angle, Attitude_Angel_P * 300, v_out/*setted_angle * 300*/);// 结果为目标角速度
    ...
    {
        Encoder_Read();
        v_out = rubo_PID(&Vel_PID, Vel, Wheel.now * 100,speed_set); // 结果为放大300倍的目标角度 350
        v_out += setted_angle * 300; // 目标角度叠加在零点上
...
float cha=0.1;
    //-----------------------------------转向控制------------------------------------//
		float speed=Wheel.now>1+cha?1+cha:(Wheel.now<1-cha?1-cha:Wheel.now);
    Direct_Parameter =- PID_Realize1(&Direct_PID, Direct, ICM_Treated.gyro.z , Radius*Wheel.now/10);	// 转向角速度环左正右负
...
{
	 quan[i]=1+cha/2-cha/(end_line-start_line)*(i-start_line);
}
...
//    excursion[17] = 0.000f;
    for (Ysite = start_line; Ysite < end_line ; Ysite ++)
    {
        sumcenter += excursion[Ysite]/**quan[Ysite]*/;
    }
    point_center0 = (int32)(sumcenter / 10 * 2.0f);
    point_center1 = point_center0;
    point_center2 = point_center1;
