...
void Timer_IT(void)  //TM6中断
{
    ...
    {
        Encoder_Read();
        v_out = rubo_PID(&Vel_PID, Vel, Wheel.now * 100,300); // 结果为放大300倍的目标角度 350
        v_out += set_angle * 300; // 目标角度叠加在零点上
        v_out = range_protect(v_out, 7500, 13500);  // -5*300=-1500 -65*300=19500, 33~~40
    }
    //-----------------------------------转向控制------------------------------------//
    Direct_Parameter = -PID_Realize1(&Direct_PID, Direct, ICM_Treated.gyro.z ,Wheel.now * 180/5);	// 转向角速度环左正右负   
		//    Direct_Parameter = -PID_Realize1(&Direct_PID, Direct,0,0);	 
    ...
    if (CarInfo.Protect == 'F')
    {
        ...
        {
            pwm_l = angle_v_out * 1.1-Direct_Parameter;
            pwm_l = range_protect(pwm_l, -990, 990);
