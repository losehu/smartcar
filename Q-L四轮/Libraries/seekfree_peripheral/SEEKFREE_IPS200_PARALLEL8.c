...
void ips200_showchar(uint16 x,uint16 y,const int8 dat)
{
	...
	for(i=0; i<16; i++)
	{
		...
		for(j=0; j<8; j++)
		{
            ...
            {
                if( y != lcd.cursor*16 ) ips200_wr_data16(IPS200_BGCOLOR);
                else ips200_wr_data16(IPS200_CURSORCOLOR);
            }
...
//  @since      v1.0
//  Sample usage:               ips200_showint32(0,0,x,3);//x可以为int32 uint16 int16 uint8 int8类型
//  Sample usage:               正数显示一个空格
//-------------------------------------------------------------------------------------------------------------------
void ips200_showuint32(uint16 x,uint16 y,uint32 dat,uint8 num)
{
    int8    buff[34];
    uint8   length;
