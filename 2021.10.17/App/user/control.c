...
void Timer_IT(void)  //TM6中断
{
    ...
    //    angle_v_out = PID_Increase(& Ang_Vel_PID,Ang_Vel,(int)ICM_State.gyro.y,(int)angle_out);   // 计算直立PWM
    angle_v_out = -0.35 * (ICM_State.gyro.y - angle_out);
    angle_v_out = range_protect(angle_v_out, -990, 990);
    int out = angle_v_out;

    if (TIMx_Cnt % 5 == 0 && CarInfo.Protect == 'F')
    {
        angle_out = -PID_Realize2(&Angle_PID, Angle, Attitude_Angel_P * 100, set_angle * 100);
        angle_out = range_protect(angle_out, -1500, 1500);
    }
